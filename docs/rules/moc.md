...
**Benchmark 论文特殊规则**: benchmark/evaluation 论文的 `models` 字段仅列本文提出的评估框架或基线系统,被评估的第三方系统不列入 `models`,在正文"被评估系统"段讨论。

### R4. 论文区容量与溢出

- 单个 MOC 的"相关论文"区 **≤ 50 条**。超过时拆为 sub-MOC。
- 拆分策略: 按技术路线拆(如 `零样本语音合成-AR与Codec LM.md`),总览页保留 sub-MOC 链接 + 每路线 Top 3 代表作。
- 每条论文描述 **≤ 1 行**,目标 **15-40 字**(论文名 + 年份 + 机构 + 一句话贡献)。超过 60 字的描述应精简。
- 超过 30 条时,按子技术路线分组(如 AR 路线 / NAR 路线 / Flow 路线),避免长列表。

**拆分操作指南**:
1. **触发条件**: 论文区 > 40 条(WARNING)或 > 50 条(应立即拆分)
2. **拆分粒度**: 按技术路线或时间阶段拆,每个 sub-MOC 目标 20-30 条
3. **总览页保留**: 各路线 Top 3 代表作 + sub-MOC wikilink + 路线一句话描述
4. **sub-MOC 命名**: `{主题}-{路线名}.md`,如 `零样本语音合成-AR与CodecLM.md`
5. **scope 继承**: sub-MOC 的 scope 声明须明确与父 MOC 的关系

**自动检测**: lint L15 在 full-check 时自动检测每个 MOC 的论文引用数:
- `> 40`: WARNING — 准备拆分计划
- `> 50`: 应立即拆分
- `> 80`: CRITICAL — 必须立即拆分
...
- **inline 年份一致**: 论文描述中的年份文本(如"2024"、"(2024)")也必须与 frontmatter `year` 一致,不可凭印象填写。
