...
- `datasets` 字段**至少**填写核心实验数据集(论文主实验使用的数据集),不可留空
- `models` 字段必须含本文模型 + 对比 baseline;后继系统仅在正文讨论时才列入
- `concepts` 字段必须含标题中的核心概念

### 特殊论文类型标注

| 论文类型 | 标注要求 |
|----------|----------|
| 无定量评估 | 实验节开头显式声明"本文无标准定量指标",改用定性分析/case study 格式 |
| Feasibility study | 审阅增加"完整性"维度: 是否覆盖了问题定义、可行方案、初步验证、局限讨论 |
| Benchmark 论文 | `models` 仅列本文提出的模型/系统,被评估的外部系统放正文讨论 |
