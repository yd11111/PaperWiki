...
- [[论文笔记/UNISON|UNISON]] (Li et al., CUHK, 2026): Pure WER 1.27% (test-en), Pure CER 0.92% (test-zh), ZS WER 1.50% (test-en), ZS CER 0.89% (test-zh); 621-732M 统一音频生成+编辑系统,layer-wise deep LLM fusion + MM-DiT flow matching,无 phoneme encoder,多任务训练未降低 TTS 质量
- [[论文笔记/FlowTTS-GRPO|FlowTTS-GRPO]] (Wang et al., Tongyi Lab, Interspeech 2026): CV3 FM-GRPO SS2 0.859 (test-zh), SS1 0.804 (test-zh, 首次超越闭源 Seed-TTS 0.796), CER 1.26% (test-zh); F5-TTS FM-GRPO SS2 0.827 (test-zh), CER 1.55% (test-zh), WER 1.73% (test-en); ODE→SDE 转换 + GRPO 直接微调 FM 模型
- [[论文笔记/ZONOS2|ZONOS2]] (Clark et al., Zyphra, 2026): SIM 47.60 (test-en), WER 2.05% (test-en), SIM 58.20 (test-zh), WER 2.55% (test-zh), WER 11.15% (test-zh-hard); 8B MoE (900M active) decoder-only codec LM,首个开源 MoE TTS,ECAPA-TDNN + LDA speaker embedding
- [[论文笔记/dots.tts.edit|dots.tts.edit]] (Wang et al., 2026): en WER 1.39% / SIM 0.760, zh CER 0.96% / SIM 0.805, zh-hard CER 6.75% / SIM 0.790; 作为**编辑后训练能力保持性**测试——相对最强 dots.tts 基座变体退化 ≤0.29% 绝对 WER/CER、≤0.011 SIM,说明大比例 TTS replay (48:4:4:2:1) 使编辑适配几乎不损零样本 TTS 能力 [dots.tts.edit Table 7]
