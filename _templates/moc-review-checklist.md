...
- [ ] 演进 section 确实使用了关系符号(不是纯文本列表) `[R7]`
- [ ] SOTA 表(如有)满足 R6 全部标注要求(benchmark/metric/数据来源/日期/可比性声明) `[R6]`
- [ ] 按模型/概念浏览区链接优先指向实体页(有实体页的优先用实体页链接) `[R1]`

### 不腐烂

- [ ] 每条论文的年份 section 归属与该论文 frontmatter `year` 字段一致 `[R5]`
- [ ] 论文描述中的 inline 年份文本也与 frontmatter `year` 一致 `[R5]`
- [ ] 自动刷新后运行 year-mismatch 验证(抽样检查 ≥10 条) `[R5]`
...
- [ ] 超过 30 条时按子技术路线分组 `[R4]`
- [ ] 超过 80 条时触发拆分建议(CRITICAL) `[R4]`
- [ ] 每条论文描述 ≤ 1 行,目标 15-40 字 `[R4]`
- [ ] 人工备注区块存在且未被覆盖 `[运维]`
...
| missing-evolution-markers | 演进脉络缺少关系符号(纯文本列表) |
| year-section-batch-drift | 批量刷新时年份映射系统性偏移 |
...
| 2026-06-09 | v1.1: +inline 年份一致; +year-mismatch 自动验证; +拆分触发(>80 CRITICAL); +30 条分组; +演进关系符号检查; +实体页链接优先; +2 新 issue types (missing-evolution-markers, year-section-batch-drift) | 51 条 learning signals 批量消化 |
