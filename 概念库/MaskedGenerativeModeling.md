...
key_papers: ["[[论文笔记/SoundStorm|SoundStorm]]", "[[论文笔记/NaturalSpeech3|NaturalSpeech 3]]", "[[论文笔记/MaskGCT|MaskGCT]]", "[[论文笔记/Metis|Metis]]", "[[论文笔记/Chatterbox-Flash|Chatterbox-Flash]]", "[[论文笔记/DiSTAR|DiSTAR]]", "[[论文笔记/OmniVoice|OmniVoice]]", "[[论文笔记/LLaDA-TTS|LLaDA-TTS]]", "[[论文笔记/PALLE|PALLE]]", "[[论文笔记/TaDiCodec|TaDiCodec]]", "[[论文笔记/ChainOfDetails|Chain-of-Details]]", "[[论文笔记/KineticOptimalTTS|GibbsTTS]]"]
origin_paper: "Chang et al., MaskGIT: Masked Generative Image Transformer, CVPR 2022"
related_concepts: ["[[SpeechTokenizer]]", "[[ResidualVectorQuantization]]"]
...
- [[SpeechTokenizer]]: masked generative modeling 要求离散 token 作为输入/输出
- [[ResidualVectorQuantization]]: S2A 阶段按 RVQ 层逐层生成
- Diffusion Model: 连续空间的类似迭代精化思路 (masked generative 是离散空间的对应物)

## 演进

BERT (2019, masked LM for understanding) → MaskGIT (2022, masked generation for images) → SoundStorm (2023, acoustic token generation) → MaskGCT (2024, full TTS pipeline) → Block Diffusion (Arriola et al., 2025) + Chatterbox-Flash (2026, block-causal masked denoising for streaming TTS with prior-calibrated scoring) → OmniVoice (2026, 单阶段 text-to-multi-codebook, full-codebook random masking 打破 per-layer 惯例, 600+ 语言) → LLaDA-TTS (2026, ELBO-derived 1/t masked diffusion, AR→bidirectional 迁移, 免训练 speech editing) → PALLE (2025, pseudo-autoregressive: 在双向 masked generative transformer 中注入 span-level 时序约束,固定步数 O(1) 生成,兼具 AR 时序建模和 NAR 并行效率; WER-W 2.23% vs MaskGCT 4.22%, RTF 0.06) → Chain-of-Details (Ma & Cartwright, 2026, temporal coarse-to-fine: 在时间分辨率维度做渐进精化而非 RVQ 层级维度, 共享 decoder 跨 temporal level, 263M params 达 3.09% WER) → GibbsTTS (Yang et al., 2026, 首个 MI-DFM for TTS: 用 metric-induced path 替代 mask-source path,在同架构/同数据 controlled comparison 中全面超越 masked DFM 和 masked DD baselines; 399M DiT, UTMOS 3.651, SIM 0.743 on Seed test-en) → Metis (Wang et al., 2025, 首个 masked generative pre-training for unified speech generation: 在 300K h 无标注语音上做无条件 MGM 预训练,通过 fine-tuning 适配 5 种任务 (TTS/VC/SE/TSE/L2S) 均达 SOTA,LoRA 32M params + 1K h 即可获得竞争性能,证明 MGM 可作为语音基座模型预训练范式) → [[论文笔记/StellarTTS|StellarTTS]] (Luo et al., 2026, 对 alignment-free masked generative 路线的逆向修正: 指出 MaskGCT/E2/F5 抛弃 phone-level alignment 导致商业不可接受的高 WER,用 sparse temporal embedding[每音素只在时长中点放锚 token、其余填 padding]软性找回对齐鲁棒性,配合单阶段 semantic-aware codec,83M 参数 RTF 0.08 面向手机端; Seed test-hard WER 7.47 最低但 SIM-o 0.712 偏低)
