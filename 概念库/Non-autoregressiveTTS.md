...
key_papers: ["[[论文笔记/VITS|VITS]]", "[[论文笔记/E2TTS|E2 TTS]]", "[[论文笔记/OmniVoice|OmniVoice]]", "[[论文笔记/LLaDA-TTS|LLaDA-TTS]]", "[[论文笔记/SESD|SESD]]", "[[论文笔记/VoiceFlow|VoiceFlow]]", "[[论文笔记/MetaLearningTTS7000Languages|Meta Learning TTS 7000 Languages]]", "[[论文笔记/VeryAttentiveTacotron|Very Attentive Tacotron (Battenberg et al., 2025)]]", "[[论文笔记/PROEMO|PROEMO]]", "[[论文笔记/RADKA-CSS|RADKA-CSS]]", "[[论文笔记/Low-ResourceForwardTacotron|Low-Resource ForwardTacotron (Kayyar et al., 2025)]]", "[[论文笔记/FlexSpeech|FlexSpeech]]", "[[论文笔记/OpenOmni|OpenOmni]]", "[[论文笔记/F5R-TTS|F5R-TTS]]", "[[论文笔记/GSA-TTS|GSA-TTS]]", "[[论文笔记/RapFlow-TTS|RapFlow-TTS]]", "[[论文笔记/FMSD-TTS|FMSD-TTS]]", "[[论文笔记/ZipVoice|ZipVoice]]", "[[论文笔记/DS-TTS|DS-TTS]]", "[[论文笔记/FNH-TTS|FNH-TTS]]", "[[论文笔记/DiFlow-TTS|DiFlow-TTS]]", "[[论文笔记/ZipVoice-Dialog|ZipVoice-Dialog]]", "[[论文笔记/Cross-LingualF5-TTS|Cross-Lingual F5-TTS]]", "[[论文笔记/EmoSSLSphere|EmoSSLSphere]]", "[[论文笔记/DiaMoE-TTS|DiaMoE-TTS]]", "[[论文笔记/TMD-TTS|TMD-TTS]]", "[[论文笔记/Flamed-TTS|Flamed-TTS]]", "[[论文笔记/MAVE|MAVE]]", "[[论文笔记/WavTTS|WavTTS]]"]
origin_paper: "Xu Tan et al., A Survey on Neural Speech Synthesis, 2021"
related_concepts: ["[[Attention-basedTTS]]", "[[DurationPredictor]]", "[[MelSpectrogram]]", "[[Text-to-SpeechPipeline]]"]
...
- [[Attention-basedTTS]]: NAR TTS 的前身,FastSpeech 从其 attention 中提取 duration
- [[DurationPredictor]]: NAR TTS 的核心组件,桥接长度不匹配
- [[MelSpectrogram]]: NAR 模型的输出目标
- [[ProsodyModeling]]: variance adaptor (pitch, energy) 提升 NAR 韵律
...
Tacotron (AR+Attention, 2017) → FastSpeech (NAR+Duration, 2019) → FastSpeech 2 (直接训练, 2020) → Glow-TTS/VITS (NAR+Flow, 2020-21) → **Voicebox** (Meta, 2023; OT-CFM + text-guided infilling, 首次在大规模数据 60K hrs 上证明 NAR flow matching 超越 AR 方案 VALL-E) → **LLM-based TTS 回归 AR** (VALL-E, 2023) → Masked generation (NAR 新形态, MaskGCT, 2024) → Hybrid: LLM + NAR Flow (CosyVoice, 2024) → OmniVoice (2026, 单阶段 discrete NAR + LLM 初始化, 首个成功将 AR LLM 权重迁移至 NAR 架构的 TTS, 600+ 语言) → LongCat-AudioDiT (2026, 纯 NAR diffusion 在 waveform latent 空间建模, SIM SOTA on Seed) → [[论文笔记/UltraLowLatencyTTS|UltraLowLatencyTTS]] (Su et al., 2026, FS2 + Mimi 32-layer RVQ 端到端流式, depth-wise sequential decoding 在帧内逐层条件预测, RTF 0.0033) → [[论文笔记/X-Voice|X-Voice]] (Xu et al., 2026, 0.4B NAR CFM 多语言 TTS, 30 语言 transcript-free 零样本跨语言克隆, Dual-Level LID + DCFG, RTF 0.073 vs AR Qwen3-TTS 1.754, WER 竞争性能) → [[论文笔记/Qwen-Audio-3.0-Gen-Preview|Qwen-Audio-3.0-Gen-Preview]] (Alibaba Token Foundry, 2026, 纯 NAR DiT + 共享连续 VAE, 把生成目标从单一语音扩展到异质混合场景——对话/环境音/音效/长程结构音同轨生成; 在 Seed-TTS-Eval SIM 三子集全最高 0.805/0.819/0.808, 但 WER 非最优 [Table 2]) → [[论文笔记/StellarTTS|StellarTTS]] (Luo et al., 2026, 移动端 masked generative NAR: 83M LLaMA decoder + 单阶段 semantic-aware codec, RTF 0.08 面向骁龙 SM8650; 用 sparse temporal embedding 替代 length regulator, 兼顾 alignment 鲁棒性与韵律自由度, Seed test-hard WER 7.47 最低)
