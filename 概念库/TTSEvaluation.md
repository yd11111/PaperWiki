...
### NVV-SuperBench: 45-Type NVV Multi-Axis Benchmark

[[论文笔记/NVV-SuperBench|NVV-SuperBench]] (Xue et al., NJU/HKUST, 2026) 将 NVV 评估从 NV-Bench 的 14 类扩展至 45 类 (6 大类: Respiratory/Throat/Laughter Spectrum/Crying Spectrum/Emotional Vocalizations/Oral),并引入 multi-axis evaluation protocol 将语音质量与 NVV-specific controllability/placement/perceptual salience 解耦评估。4,500 条双语 (EN+ZH) 数据,15 个系统 (7 prompt-based + 8 tag-based)。核心方法创新: GT-conditioned verification — 给 LLM verifier 提供目标 NVV 类型约束做 constrained editing,比 open-ended 检测更可靠 [§2.4.1]。关键发现: (1) NVV controllability 与 speech quality decouple (CosyVoice 2 ZH: quality 4.35 但 NVV accuracy 1.65) [Table 4]; (2) low-SNR oral cues (tsk, lipsmack) 和 long-duration affect (crying, wail) 是持续瓶颈 [Fig. 2]; (3) tag-based 存在 breadth-correctness trade-off (ChatTTS coverage 0.02 但 F1 虚高) [Table 3]。与 NV-Bench 的关系: NV-Bench 用 PCER 单指标评估 NVV 生成,NVV-SuperBench 提供多轴 (objective + subjective + LLM-judge) 综合评估框架。详见 [[论文笔记/NVV-SuperBench|NVV-SuperBench]]。
