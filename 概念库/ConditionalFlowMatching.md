...
- [[论文笔记/STAR-VAE|STAR-VAE]] (Liu et al., ICML 2026): 其生成模块 STAR-Gen 把 CFM 用于**通用音频(sound+music)T2A** 生成,创新在于用 **LLM decoder(Qwen3-0.6B)作为序列级 flow matching 向量场估计器** + 混合 attention(text causal / audio 双向),而非独立 DiT 或 per-token head。训练目标为标准 CFM(logit-normal t, 回归 (z1-z0)),但在 [[StructuredTopology-AwareRegularization|STAR]] 重塑的结构化连续 latent 上运作。关键发现:CFM 生成质量强依赖底层 tokenizer 结构性——STAR-Gen 换掉 STAR-VAE latent(用 SAO-VAE/ϵar-VAE)后 FDopenl3 从 55.8 升至 67.4/76.45 [STAR-VAE Table 2]。CFG scale 3.0 + 24 步最优。
- [[论文笔记/Locodec|Locodec]] (Luo et al., ByteDance, 2026): 用 **球面 flow matching (SFM)**——Riemannian FM 的球面特例 [Chen & Lipman 2023]——作为球面连续 token 的 bridge。三个有用性质 [Locodec §3.5]: (1) 高维近正交下 SFM 测地插值 ≈ VP 三角插值 cos(πt/2)x0+sin(πt/2)x1,近似保范数;(2) SFM 的 ∥x_t∥ 沿 bridge 恒定,而线性插值 norm 与时间强耦合会加剧曝光偏差;(3) **direction-only 参数化**: 常规 full-velocity MSE 会诱导 under-stepping(固定方向下 MSE 最优速度幅值 a*=s·cosψ<真值,模型用缩步长解释方向不确定性),故只用预测端点定义切向、把速度幅值固定为 πR/2,只监督 cosine 方向损失。这是 CFM 在**非欧(球面)几何**上的适配,提示 x-pred/v-pred 之争在切空间约束下会退化(有效端点参数化)。
- [[论文笔记/SwanTale|SwanTale]] (ByteDance, 2026): 用 **non-causal flow-matching DiT** 作为唯一生成器(无 LLM backbone),在 SwanVAE 的 48kHz/25Hz **波形-latent** 空间上做 flow matching,统一 instruct(full caption)与 zero-shot(content caption + 参考音 prompt 帧)两任务——两者只差 caption 输入和 context mask [SwanTale Eq 6-9]。与 CosyVoice 系列 LLM→token→CFM renderer 两阶段不同,属 F5/MegaTTS3 的纯 DiT 阵营;并把 DiT 的 FFN 每隔一层换成 Unified MoE(见 [[MixtureofExpertsforAudioGeneration]]),叠加 Engram caption cross-attention 与 reward-conditioned quality control。
