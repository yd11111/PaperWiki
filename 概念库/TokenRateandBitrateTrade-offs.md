...
key_papers: ["[[论文笔记/Survey-DiscreteAudioTokens|Survey-Discrete Audio Tokens]]", "[[论文笔记/DAC|DAC]]", "[[论文笔记/SoundStream|SoundStream]]", "[[论文笔记/Llasa|Llasa]]", "[[论文笔记/TADA|TADA]]", "[[论文笔记/LM-SPT|LM-SPT]]", "[[论文笔记/C2F-LM|C2F-LM]]", "[[论文笔记/Dragon-FM|Dragon-FM]]", "[[论文笔记/TaDiCodec|TaDiCodec]]", "[[论文笔记/VARSTok|VARSTok]]", "[[论文笔记/MSR-Codec|MSR-Codec]]", "[[论文笔记/BridgeCode|BridgeCode]]", "[[论文笔记/FlexiSLM|FlexiSLM]]"]
origin_paper: "Mousavi et al., Discrete Audio Tokens: More Than a Survey!, TMLR 2025"
related_concepts: ["[[ResidualVectorQuantization]]", "[[Single-codebookvsMulti-codebook]]", "[[AudioTokenizerTaxonomy]]", "[[QuantizerDropout]]"]
...
低 token rate 对 LM 有巨大优势: 更短序列 → 更快推理 → 更低内存 → 更好的长距离建模。这驱动了 [[Single-codebookvsMulti-codebook]] 中向更少码本的趋势。
...
### Scalable Bitrate
通过改变活跃码本数量实现多档比特率:
- 通过 [[QuantizerDropout]] 训练单一模型支持多种比特率
...
- **原生高维单 token vs 分组低维**: [[论文笔记/Locodec|Locodec]] (Luo et al., ByteDance, 2026) 给这个 trade-off 提出第三种解法。前作 DiTAR 走"分组低维 product-structured"路线(高帧率低维 token 分组后压成一个 embedding 进 LM,推理期用 local DiT 解回一组);Locodec 质疑: 既然生成器内部反正要做压缩-解压,不如把它搬进 tokenizer,直接产**原生 8Hz/768 维高维连续单 token**,让 AR 在 token 率上工作、省掉推理期 local DiT [Locodec §2.2]。作者论证 8Hz(每 token≈125ms 接近音素时长)本身即提供"语义归纳偏置",低帧率利于内容聚合与 AR 长程稳定 → Seed-TTS-eval ZH WER 0.95%(全表最低),但代价是 SIM 明显落后(单一 reconstruction-first token 偏内容、弱细粒度声学)[Locodec Table 5]。这为本页 trade-off 增加了"连续 AR 场景下帧率同时是效率参数+语义-声学分辨率钮"的新维度。
...
- [[ResidualVectorQuantization]]: 比特率 = frame_rate x N_q x log2(K) 中的 N_q 和 K
- [[Single-codebookvsMulti-codebook]]: 降低 token rate 的核心策略
- [[QuantizerDropout]]: 实现 scalable bitrate 的训练技巧
- [[AudioTokenizerTaxonomy]]: 比特率是 taxonomy Axis 2 的子维度
