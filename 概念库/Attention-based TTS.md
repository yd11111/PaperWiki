...
- [[Non-autoregressive TTS]]: 用 duration prediction 替代 attention,实现并行生成
- [[Duration Predictor]]: 从 attention-based 模型的对齐中提取 duration 标签
- [[Mel Spectrogram]]: attention-based TTS 的输出目标
- [[Prosody Modeling]]: 通过 reference encoder / style token 增强韵律

## 演进

Tacotron (2017) → Tacotron 2 (2017.12) → TransformerTTS (2018) → **FastSpeech 取代 attention** (2019) → DurIAN/Non-Att Tacotron (混合形态) → 现代 LLM-TTS 回归 AR 但用 discrete tokens (VALL-E, 2023)
