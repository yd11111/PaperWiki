...
arxiv_id: "2506.06252"
source: "Sources/Lightweight Prompt Biasing.pdf"
...
> [!info] KB 背景 (基于 2 个待确认实体页: [[Whisper]], [[LLM-enhanced ASR]])
> 自动生成,不保证完整覆盖所有相关知识。两页均为 pending-review,仅供参考。
>
> **谱系定位**: 本文属于 ASR contextual biasing 方向,目标是提升 E2E ASR 对稀有/领域特定实体的识别准确率。这一方向与 [[LLM-enhanced ASR]] 的 Rescoring/GER 路线平行但方法不同: LLM-enhanced ASR 在 ASR 后端用 LLM 重排序或生成纠错;本文在 ASR 模型内部通过 prompt + multi-task 直接引入上下文偏置,无需额外模型。
>
...
>
> 检索命中: [[Whisper]][待确认], [[LLM-enhanced ASR]][待确认] | 过滤: 无 | 未命中但可能相关: 无
