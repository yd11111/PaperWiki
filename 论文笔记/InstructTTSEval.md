...
> - **指标**: 商用最佳 gemini-pro EN-Avg 71.1%, 开源最佳 Parler-TTS-mini EN-Avg 54.3%; 参考音频 EN-Avg 84.3%; 人机一致率整体 79.0% (APS 87%, DSD 79%, RP 71%) [Table 4-6]
...
| True rate (%) | gemini-flash | 80.1 | 88.7 | 39.3 | 50.4 | EN-subset | [Table 5] |
| True rate (%) | gemini-pro | 67.2 | 80.3 | 28.6 | 46.9 | EN-subset | [Table 5] |
| True rate (%) | gpt-4o-mini-tts | 54.8 | 68.5 | 31.2 | 45.7 | EN-subset | [Table 5] |
| True rate (%) | hume | 54.3 | 71.1 | 31.4 | 47.6 | EN-subset | [Table 5] |
| True rate (%) | VoxInstruct | 54.9 | 57.0 | 30.9 | 38.2 | EN-subset | [Table 5] |
| True rate (%) | Parler-TTS-mini | 63.4 | 48.7 | — | 54.3 | EN-subset | [Table 5] |
| True rate (%) | Parler-TTS-large | 60.0 | 45.9 | — | — | EN-subset | [Table 5] |
| True rate (%) | reference_audio | 90.9 | 86.7 | 69.8 | 82.5 | ZH-subset | [Table 6] |
| True rate (%) | gemini-flash | 77.3 | 85.4 | 42.6 | — | ZH-subset | [Table 6] |
| True rate (%) | gemini-pro | 75.5 | 84.8 | 47.5 | — | ZH-subset | [Table 6] |
| True rate (%) | gpt-4o-mini-tts | 46.0 | 51.1 | — | — | ZH-subset | [Table 6] |
| True rate (%) | VoxInstruct | 47.5 | 52.3 | — | — | ZH-subset | [Table 6] |
| 一致率 (%) | Human-Gemini (EN) | 86 | 78 | 66 | 76.7 | EN-subset | [Table 4] |
| 一致率 (%) | Human-Gemini (ZH) | 88 | 80 | 76 | 81.3 | ZH-subset | [Table 4] |

### 关键发现

1. **闭源远超开源**: 商用模型 (gemini, gpt-4o-mini-tts, hume) 在所有任务上显著优于开源系统,展现更精确的声学控制 [§4.3] [论文原文]。

2. **RP 任务难度最高**: 所有系统在 RP 任务上表现最差 (最高 39.3%),说明从角色/场景推理声学表达仍是巨大挑战 [§4.3]。

3. **参考音频也非满分**: 参考音频在 RP 任务上仅 67-70%,说明 bottom-up 生成的 RP 指令与原始音频之间本身存在偏差 [agent 解读]。

4. **VoxInstruct 在开源中最佳但有短板**: VoxInstruct 在 DSD/RP 上领先其他开源系统,但 APS 上较弱,可能因为处理长输入能力有限 [§4.3] [论文原文]。

5. **开源系统在特定维度有优势**: 部分开源系统 (如 Parler-TTS-Large) 在音色模拟 (如老年声音、儿童声音) 上表现出色,说明音色灵活性和情感表现力是正交能力 [§4.4] [论文原文]。

6. **Self-preference bias 风险**: 论文承认 Gemini 做 judge 可能偏好 Gemini TTS 的输出 [§4.3] [论文原文]。
...
1. **评估器选择的局限**: 用 Gemini 评 Gemini 的 TTS 输出,存在无法消除的 self-preference bias。论文虽承认但未做任何缓解 (如引入第三方 judge 做 ablation)。这使 Gemini 系列的高分需要打折看。

2. **True/False 粒度**: 对于 12 维特征的指令,仅给出一个二分判定,丧失了大量诊断信息。比如一个系统在 11 维匹配、1 维不匹配,和完全不匹配的系统得到同样的 False,这降低了 benchmark 的区分度。

3. **参考音频 RP 仅 67-70%**: 这说明 RP 指令本身与音频的对齐就有问题,使得 RP 子任务的评估可靠性存疑。这可能是 bottom-up 构造的固有局限 — 从音频反推的角色描述未必能忠实还原原始表演。
