...
arxiv_id: "2507.04598"
source: "Sources/Multi-StepHierarchicalED.pdf"
...
concepts: ["[[EmotionControlinTTS]]", "[[ProsodyModeling]]", "[[Non-autoregressiveTTS]]", "[[F0Modeling]]", "[[DurationPredictor]]"]
models: []
tasks: []
datasets: []
...
**已有认知**:
- [[EmotionControlinTTS]] [待确认] 记录了情感控制从 one-hot label → embedding → 多尺度层级 → DPO/RLHF → 球面向量 → LLM 文本描述的完整演进线。该页已收录 MsEmoTTS 和 EmoSphere-TTS 等工作,但尚未覆盖本文的"多步预测"范式。
- [[ProsodyModeling]] 指出韵律的物理维度包括 Duration/Pitch/Energy/Pause,FastSpeech 2 的 variance adaptor 是显式韵律建模的标杆。本文正是在 FastSpeech 2 的 variance adaptor 框架上扩展情感维度。
- [[Non-autoregressiveTTS]] 描述了 FastSpeech 2 架构: Encoder → Duration Predictor → Length Regulator → Pitch/Energy Predictor → Decoder,本文的两种集成方式都建立在此基础上。
- [[DurationPredictor]] 记录了 variance adaptor 中的 duration 预测机制,本文在此基础上增加了层级 ED 预测分支。
- [[SpeakerEmbedding]] 中 Resemblyzer (GE2E) 是本文多说话人场景使用的 speaker encoder。
...
> 检索命中: [[ProsodyModeling]]✓, [[SpeakerEmbedding]]✓ | 过滤: [[EmotionControlinTTS]](pending-review), [[Non-autoregressiveTTS]](pending-review), [[F0Modeling]](pending-review), [[DurationPredictor]](pending-review) | 未命中但可能相关: [[StyleTransferinTTS]]
